...
## Fase atual: Fase 8 — concluída e validada; UrbanFlow Core v1 congelado
...
## Fase 8 — Congelamento e baseline oficial

### Status

`Concluída e validada — UrbanFlow Core v1 congelado como baseline oficial.`

### Etapa 1 — Auditoria (concluída, aprovada com ressalvas)

Auditoria de arquitetura executada por leitura direta do repositório
(sem alteração de arquivos). Cobriu: mapa de arquitetura, módulos
Core vs. COMLURB, contratos públicos, dependências entre módulos,
arquivos legados/experimentais, riscos de acoplamento, e proposta de
perímetro (A–E).

Confirmado externamente pela proprietária do produto: as tags
`fase-7b-concluida` e `fase-7c-concluida` e as respectivas releases
("Fase 7B concluída — Piloto Live × Snapshot AR" e "Fase 7C concluída
— Piloto Live × Snapshot Engenharia/DTE", cada uma com o pacote final
da fase correspondente anexado) foram criadas manualmente no GitHub,
fora deste checkout — por isso não aparecem no conteúdo de arquivos
auditado. Este documento registra a confirmação, mas não recomputa
nem verifica essas tags/releases a partir deste checkout, que não
contém `.git`.

Achados adicionais da auditoria, ainda não corrigidos (fora do escopo
desta etapa):

- `README.md` e `GUIA_OPERACIONAL.md` descrevem uma estrutura de
  pastas anterior à atual (`dte/`, `sms/`, `sme/`, `radar-pre/` como
  pastas de raiz), divergente da estrutura real do repositório.
- As 13 ADRs mencionadas no histórico do projeto existem apenas como
  citações soltas em comentários de código (`ADR-001`, `ADR-004`,
  `ADR-005`, etc., em `snapshot/README.md`,
  `assets/components/hub-rules-ar.js`,
  `assets/components/hub-ingest-model.js`,
  `assets/components/hub-ingest-reader.js` e neste próprio documento)
  — não há um documento consolidado de ADRs no repositório.
- `assets/components/hub-ingest-adapter-ar.js` e
  `assets/components/hub-rules-ar.js` documentam, no próprio
  cabeçalho, que a versão de `hub-utils.js` hoje publicada diverge de
  uma versão anterior referida como "Fase 2" — duas linhagens não
  reconciliadas do mesmo arquivo.
- `hub-state.js` genérico não existe no repositório;
  `hub-state-ar.js` mantém taxonomia local por esse motivo, conforme
  o próprio cabeçalho do arquivo já registrava.

Perímetro do Core formalizado em
`docs/architecture/CORE_PERIMETRO.md` (novo nesta etapa), distinguindo
A (infraestrutura reutilizável), infraestrutura "reutilizável com
acoplamentos ainda existentes" (`hub-sources.js`, `hub-rules.js`,
componentes de UI, CSS), B (infraestrutura compartilhada ainda
acoplada), C (módulos específicos da instância), D (painéis e
produtos finais) e E (histórico/teste/documentação, fora do
baseline).

### Etapa 2 — Regressão oficial (em andamento)

Escopo autorizado e executado nesta rodada: criação de
`docs/architecture/CORE_PERIMETRO.md`; alteração de `package.json`
(novos scripts `test:fase7a`, `test:fase7b`, `test:fase7c` e
`test:regression`, que encadeia as sete suítes na ordem
selftest→Fase4→Fase5→Fase6→Fase7A→Fase7B→Fase7C e interrompe no
primeiro erro; `npm test` passou a apontar para `test:regression`,
preservando todos os scripts individuais); alteração de
`.github/workflows/snapshot.yml` (o passo de testes obrigatórios
antes da captura de snapshot agora executa as sete suítes em vez de
quatro; nenhuma outra parte do workflow — agenda, permissões,
retenção, diretórios, lógica de captura, publicação de artifact — foi
alterada); esta seção do próprio
`docs/architecture/IMPLEMENTATION_STATUS.md`.

**Achado crítico identificado nesta rodada e já corrigido (rodada
corretiva mínima, ver abaixo):** `testes/testar-fase7b.js` contém uma
suíte de integridade que compara o hash SHA-256 atual de 16 arquivos
protegidos contra um hash esperado, fixado no momento do fechamento da
Fase 7B — entre eles, `package.json`. A alteração de `package.json`
autorizada e executada nesta etapa (adição dos scripts de Fase
7A/7B/7C e `test:regression`) mudou esse hash por definição. Isso
gerou 145/146 em Fase 7B numa primeira execução, com Fase 7C sequer
chegando a rodar dentro da cadeia (a suíte interrompeu corretamente no
primeiro erro).

### Rodada corretiva — reconciliação do hash protegido de `package.json`

Autorizada explicitamente e de escopo mínimo: em
`testes/testar-fase7b.js`, o valor esperado de `package.json` na lista
`HASHES_ESPERADOS` foi atualizado para o SHA-256 do `package.json`
final da Fase 8 (pós inclusão dos scripts de Fase 7A/7B/7C), com um
comentário curto registrando a migração de baseline. Nenhum outro hash
da lista foi alterado; `package.json` não foi removido da lista de
proteção; a verificação continua sendo uma comparação estática contra
um valor fixo (não virou aceitação dinâmica do hash atual, nem foi
enfraquecida, pulada ou condicionada).

Após a correção, `npm test` executa as sete suítes na ordem
selftest→Fase4→Fase5→Fase6→Fase7A→Fase7B→Fase7C e termina com código
de saída 0:
...
Confirmado por diff recursivo contra o checkout original: nesta
rodada, apenas cinco arquivos foram criados ou alterados —
`docs/architecture/CORE_PERIMETRO.md`, `package.json`,
`.github/workflows/snapshot.yml`,
`docs/architecture/IMPLEMENTATION_STATUS.md` e
`testes/testar-fase7b.js`. O efeito colateral conhecido de
`testar-fase5.js` sobre
`engenharia-operacional/piloto/saida-canonica-exemplo.json` foi
restaurado ao estado original após a execução dos testes.
`node_modules` (criado por `npm ci` para rodar a regressão) foi
removido ao final, por não fazer parte do checkout original.

### Não executado nesta rodada

- Reescrita de `README.md` e `GUIA_OPERACIONAL.md`.
- Criação de `ADRS.md` ou reconstrução de ADRs a partir de comentários.
- Correção de `hub-utils.js` ou criação de `hub-state.js`.
- Separação de conteúdo de `hub-sources.js` ou parametrização de tema.
- Qualquer movimentação de `balanco-receita/` ou de qualquer outro
  arquivo além do autorizado.
- Criação de tag, release ou ZIP.
- Declaração de `urbanflow-core-v1` como criado.
- Congelamento final do baseline.

### Próxima ação autorizada (registro histórico desta rodada)

Nenhuma. Aguardar autorização explícita para qualquer etapa seguinte,
incluindo o congelamento final da Fase 8.

### Etapa 3 — Congelamento final (concluída e validada)

Autorizada explicitamente. Antes de qualquer alteração, reconfirmado
por execução direta: `npm test` continua executando as sete suítes na
ordem selftest→Fase4→Fase5→Fase6→Fase7A→Fase7B→Fase7C; total 604/604;
código de saída 0; `.github/workflows/snapshot.yml` continua
executando a regressão integral antes da captura de snapshot;
`package-lock.json` permanece bit-a-bit idêntico ao checkout original
(hash MD5 conferido antes e depois de `npm ci`); `node_modules` não
faz parte do checkout final; diff recursivo contra o checkout original
confirma que somente os cinco arquivos já autorizados na Fase 8
differiam até este ponto; nenhuma mudança fora do escopo permanece;
Fases 7B e 7C seguem documentadas como concluídas e validadas nas
seções acima; as tags `fase-7b-concluida`/`fase-7c-concluida` e as
respectivas releases seguem registradas como criadas manualmente no
GitHub, por confirmação externa da proprietária do produto (fora
deste checkout, que não contém `.git`).

Nesta etapa foi criado `docs/architecture/CORE_BASELINE_V1.md` —
documento curto de identificação do baseline, referenciando
`CORE_PERIMETRO.md` como fonte do perímetro oficial (sem duplicá-lo),
listando garantias do baseline, contratos públicos congelados,
limitações conhecidas e política de mudança. `package.json` já possui
campo `"version": "1.0.0"` desde antes desta rodada — apenas
confirmado, não alterado.

**Perímetro:** formalizado em `CORE_PERIMETRO.md` (Etapa 1/2).
**Regressão:** automatizada em `npm test` e no workflow, sete suítes
integradas, captura de snapshot bloqueada em caso de falha.
**Hash protegido:** atualização explícita de `package.json` em
`testes/testar-fase7b.js`, documentada como migração de baseline, não
como contorno.
**Resultado:** 604/604 testes aprovados, código de saída 0.
**Escopo:** nenhum arquivo de produção, painel, adapter, regra ou
snapshot foi alterado nesta fase. Nenhuma abstração nova foi criada.
Nenhum módulo foi movido ou renomeado.

**Baseline congelado no estado atual do repositório**, com as
limitações já conhecidas — acoplamentos entre `hub-sources.js`/
`hub-rules.js`/componentes de UI e a instância COMLURB, duas linhagens
não reconciliadas de `hub-utils.js`, ausência de `hub-state.js`
genérico, `README.md`/`GUIA_OPERACIONAL.md` desatualizados,
`balanco-receita/` ainda separado de `contratos/`, ADRs não
consolidadas em documento próprio — permanecendo documentadas e **não
corrigidas** nesta fase, por estarem fora do escopo autorizado.

Nenhuma Fase 9 foi iniciada. Nenhum UrbanFlow v2 foi mencionado como
iniciado. Nenhum novo repositório foi criado.

### Próxima ação autorizada

Nenhuma. UrbanFlow Core v1 está congelado como baseline oficial no
estado descrito acima. Qualquer evolução (Fase 9, separação física de
repositório, parametrização de instância, `hub-state.js` genérico,
etc.) exige nova autorização explícita.
