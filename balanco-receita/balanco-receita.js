...
function parseRows(text){
  ...
  return data.slice(1).filter(r=>r[0]&&r[1]).map(r=>({
    secretaria:String(r[0]||"").trim(),service:standardService(r[1]),year:numeric(r[5]),billed:numeric(r[8]),paid:numeric(r[12]),debt:numeric(r[13]),
    type:String(r[16]||"").trim(),unit:standardUnit(r[17]||r[15]||r[2]),launches:1
  })).filter(r=>r.year&&r.service&&r.unit);
}

function escapeHTML(value){return String(value??"").replace(/[&<>'"]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"}[c]));}
function totals(rows){return rows.reduce((a,r)=>({billed:a.billed+r.billed,paid:a.paid+r.paid,debt:a.debt+r.debt,launches:a.launches+r.launches}),{billed:0,paid:0,debt:0,launches:0});}
...
function aggregate(rows){
  ...
  rows.forEach(r=>{
    ...
    if(!map.has(key))map.set(key,{secretaria:r.secretaria,service:r.service,unit:r.unit,type:r.type,billed:0,paid:0,debt:0,launches:0});
    const x=map.get(key);x.billed+=r.billed;x.paid+=r.paid;x.debt+=r.debt;x.launches+=1;
  });
...
function render(){
  ...
  const filtered=service?base.filter(r=>r.service===service):base;
...
function renderTable(rows){
  $("detailBody").innerHTML=rows.map(r=>`<tr><td>${escapeHTML(r.service)}</td><td><b>${escapeHTML(r.unit)}</b></td><td>${escapeHTML(money.format(r.billed))}</td><td>${escapeHTML(money.format(r.paid))}</td><td><b>${escapeHTML(money.format(r.debt))}</b></td><td>${escapeHTML(percent.format(rate(r)))}</td></tr>`).join("");
}

function populateFilter(){
  const current=$("serviceFilter").value;
  const services=[...new Set(rawRows.map(r=>r.service))].sort((a,b)=>a.localeCompare(b,"pt-BR"));
...
function init(){
  HUB.header.render("header",{systemLabel:"HUB COMLURB · INTELIGÊNCIA OPERACIONAL",title:"Performance dos Contratos de Receita",subtitle:"Débitos por período, serviço, CRE e unidade hospitalar."});
  HUB.footer.render("footer");
  document.querySelectorAll("[data-period]").forEach(btn=>btn.addEventListener("click",()=>{
    period=btn.dataset.period;document.querySelectorAll("[data-period]").forEach(b=>b.classList.toggle("active",b===btn));render();
  }));
  $("serviceFilter").addEventListener("change",render);
  $("clearFilter").addEventListener("click",()=>{$("serviceFilter").value="";period="2026";document.querySelectorAll("[data-period]").forEach(b=>b.classList.toggle("active",b.dataset.period==="2026"));render();});
